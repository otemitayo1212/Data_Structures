...
public:
  ...
  Balloon(const std::string& name_) : name(name_) {
    ...
    num_ropes = 0;
    ropes = new dsSharedPtr<Balloon>[MAX_NUM_ROPES]; // changed
  }
  ~Balloon() {
  ...
  dsSharedPtr<Balloon>& getRope(int i) const { return ropes[i]; } // changed to avoid
  ...
  void addRope( dsSharedPtr<Balloon> b) { //changed to avoid dangling pointer
    assert (num_ropes < MAX_NUM_ROPES);
    ropes[num_ropes] = b;
    num_ropes++;
  }
  // detach a rope connecting this balloon to another
  void removeRope( dsSharedPtr<Balloon> b) {  // changed
...
private:
  ...
  dsSharedPtr<Balloon>* ropes; // changed
};
