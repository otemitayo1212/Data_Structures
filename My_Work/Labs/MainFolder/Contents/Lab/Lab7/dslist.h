...
void dslist<T>::pop_front() {
    assert(size_ > 0); // Ensure the list is not empty
    Node<T>* temp = head_;
    head_ = head_->next_;
    if (head_)
        head_->prev_ = nullptr;
    else
        tail_ = nullptr; // If there was only one node
    delete temp;
    --size_;
}
...
void dslist<T>::pop_back() {
    ...
    if (tail_)
        ...
        head_ = nullptr; // If there was only one node
    delete temp;
    --size_;
}
...
void dslist<T>::destroy_list() {
    ...
    head_ = tail_ = nullptr; // Reset head and tail pointers
    size_ = 0; // Reset size
}
